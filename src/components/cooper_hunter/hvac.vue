    ...
    <template #extended>
      <Section name="HVAC">
        <List>
          <Item label="Enabled">
            <Toggle
                @click="Bus.call(device.id, device.properties.enabled ? 'disable' : 'enable')"
                :value="device.properties.enabled"></Toggle>
          </Item>
          <Item label="Set Temperature">
            <CounterInput v-on:update:value="value => Bus.call(device.id, 'setTemperature', {value: value})" v-model:value="device.properties.wdNumber" :min="15" :max="40"></CounterInput>
          </Item>
          ...
          <Item label="Mode">
            <Radio>
              <button @click.once="Bus.call(device.id, 'setMode', {value: modes.auto})" class="text-white flex">
              ...
              <button @click.once="Bus.call(device.id, 'setMode', {value: modes.heat})" class="text-white flex">
              ...
              <button @click.once="Bus.call(device.id, 'setMode', {value: modes.cool})" class="text-white flex">
          ...
          <Item label="Mute">
            <Toggle
                v-on:click="Bus.call(device.id, device.properties.mute ? 'unmute' : 'mute')"
